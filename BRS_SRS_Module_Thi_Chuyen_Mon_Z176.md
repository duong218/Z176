...
*   **Sinh mã đề ngẫu nhiên không trùng lặp**: Khi Leader nhấn "Phát hành", hệ thống sẽ sinh ra các mã đề khác nhau (mỗi mã đề gắn với 1 phòng ban cụ thể, đảo thứ tự câu hỏi). Thuật toán tạo `questionSetFingerprint` (hash tập câu hỏi) để đảm bảo không có 2 mã đề thi nào trùng khớp 100% cấu trúc câu hỏi.
...
*   **Xáo riêng từng lượt thi (AttemptQuestion)**: Ngoài việc xáo câu hỏi ở cấp mã đề (ExamCodeQuestion), khi thí sinh bấm "Bắt đầu thi" hệ thống còn sinh thêm một **snapshot xáo riêng** cho chính lượt thi đó — xáo lại thứ tự câu hỏi VÀ xáo thứ tự các phương án trả lời (Fisher–Yates shuffle). Snapshot này cố định xuyên suốt lượt thi (không bị đổi khi thí sinh tải lại trang hay mất mạng), đảm bảo mỗi lượt thi (kể cả cùng 1 người thi lại) đều có giao diện hiển thị khác nhau.
...
*   **Tự động khóa và nộp bài chống gian lận**: 
    *   Nếu thí sinh tắt trình duyệt, chuyển tab, hoặc mất kết nối internet mà **không gửi tín hiệu heartbeat hoặc thao tác bất kỳ (answer/getMyExam) lên máy chủ quá 1 phút (60 giây)**, hệ thống backend sẽ ngay lập tức **tự động nộp bài (auto-submit)** với trạng thái lượt thi chuyển sang `submitted`, lý do tự nộp là `inactive_timeout`, và chấm điểm dựa trên các đáp án đã autosave trong `CandidateAnswer`.
